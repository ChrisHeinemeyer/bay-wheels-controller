const GBFS_URL = "/gbfs-proxy/gbfs/2.3/bay/en/station_information.json";
export async function fetchStations() {
  const response = await fetch(GBFS_URL);
  if (!response.ok) {
    throw new Error(
      `GBFS fetch failed: ${response.status} ${response.statusText}`,
    );
  }
 ...
 */
export function sortStationsForMapping(stations) {
